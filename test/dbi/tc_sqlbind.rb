...
class TestSqlBindType < Test::Unit::TestCase

    def prepare(sql)
        DBI::SQL::PreparedStatement.new(nil, sql)
    end

    def test_regular_tokens
        ps = prepare("select * from foo where foo = ? and bar = '??' and baz = ?")
    ...
    def test_type_tokens
        ps = prepare("select * from foo where foo = ? and bar = '??' and baz = ?{Integer}")
