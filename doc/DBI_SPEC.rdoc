...
DBI owns the concept of a driver, database, and statement. The core
functionality for these concepts is provided by a database driver, or
DBD.

These concepts cascade in ownership: for example, a driver has
databases, a database may have statements. DBI may control many
drivers at once.

DBI uses a delegation model to communicate with its DBDs through a
series of handles. To elaborate, when someone requests a connection to
a database, DBI contacts the appropriate DBD and builds a handle in
it's name that it aligns with a DBI base class for that concept. The
...
and the base classes, intentionally so. This way the DBD's have no
